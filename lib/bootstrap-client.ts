    ...
    (e: MouseEvent) => {
      if (
        (e.target as HTMLElement)?.closest?.(
          'a,button,[role="button"],.notion-collection-card'
        )
      ) {
        isHovering = true
  ...
  for (const ch of spiritChars) {
    ...
    if (dist < RADIUS) {
      ...
      ch.el.style.textShadow =
        e > 0.3
          ? `0 0 ${Math.round(4 + e * 10)}px rgba(37,99,235,${(e * 0.4).toFixed(2)})`
          : ''
...
function initAmbientSpirits() {
  const start = () => {
    if (!ambientId) ambientId = setInterval(spawnAmbient, 6000)
  }
  const stop = () => {
    if (ambientId) {
...
function spawnAmbient() {
  ...
  const blocks = document.querySelectorAll(
    '.notion-text, .notion-quote, .notion-h1, .notion-h2'
  )
  ...
  const cls =
    Math.random() < 0.5 ? 'ambient-spirit--primary' : 'ambient-spirit--cta'
  const el = document.createElement('div')
