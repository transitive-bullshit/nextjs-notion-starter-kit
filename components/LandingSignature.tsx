...
const STRESS_FRAGMENT_SHADER = `
  ...
  void main() {
    ...
    // Safari can composite transparent WebGL fragments at full RGB intensity.
    // Keep the surface opaque and encode transparency as color intensity instead.
    float outputIntensity = clamp(alpha, 0.0, 0.9);
    gl_FragColor = vec4(color * outputIntensity, 1.0);
  }
`
...
const randomScale = (variance: number) => 1 + (Math.random() * 2 - 1) * variance

const createWave = (origin: number): Wave => {
  const tempo = randomScale(0.08)

  return {
    origin,
    startedAt: performance.now(),
    delay: WAVE_DELAY / tempo,
    duration: WAVE_DURATION / tempo,
    intensity: randomScale(0.1),
    recoil: randomScale(0.16),
    twist: randomScale(0.14)
  }
}
...
export function LandingSignature() {
  ...
  useEffect(() => {
    ...
    const frame = (time: number) => {
      ...
      for (let index = 0; index < glyphs.length; index += 1) {
        ...
        if (activeWave) {
          const delay = Math.abs(index - activeWave.origin) * activeWave.delay
          const localTime = time - activeWave.startedAt - delay
          const { duration, intensity, recoil, twist } = activeWave

          if (localTime >= 0 && localTime <= duration) {
            const progress = localTime / duration
            const pulse = Math.sin(progress * Math.PI)
            const recoilPulse = Math.sin(progress * Math.PI * 2)

            targetY -=
              pulse * clamp(wordRect.height * 0.38, 22, 72) * intensity * FORCE
            targetX +=
              recoilPulse *
              (index < activeWave.origin ? -1 : 1) *
              7 *
              recoil *
              FORCE
            targetScaleX += pulse * 0.1 * intensity * FORCE
            targetScaleY -= pulse * 0.13 * intensity * FORCE
            targetRotation += recoilPulse * 3.5 * twist * FORCE
      ...
      if (
        activeWave &&
        time - activeWave.startedAt >
          (glyphs.length - 1) * activeWave.delay + activeWave.duration
      ) {
        wave.current = null
      }
  ...
  const sendWave = (clientX?: number) => {
    ...
    wave.current = createWave(origin)
    wakeAnimation.current()
  }
