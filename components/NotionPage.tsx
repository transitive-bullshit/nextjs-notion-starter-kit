...
function wrapElementsBetweenDividers(): void {
  ...
  while (node && node !== endDivider) {
    if (isHeading(node)) {
      ...
      // 🟡 Inject SVG if sib contains links inside a 'notion-text'
      const notionLinks = sib.querySelectorAll?.('.notion-link');
