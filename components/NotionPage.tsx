...
export const NotionPage: React.FC<types.PageProps> = ({
  ...
  React.useEffect(() => {
    ...
    if (filterRootRef.current.root) {
      ...
      if (
        filterRootRef.current.container &&
        filterRootRef.current.container.parentNode
      ) {
    ...
    if (pageClass === 'notion-home') {
      ...
      cards.forEach((card) => {
        ...
        const subjectContent =
          card
            .querySelector('a.notion-link')
            ?.textContent?.toLowerCase()
            .match(/\(([^)]+)\)/)?.[1] || ''
        const matchesDepartment =
          !department || subjectContent.includes(department.toLowerCase())
  ...
  function wrapElementsBetweenDividers(): void {
    ...
    startDivider.parentElement?.insertBefore(
      gridWrapper,
      startDivider.nextSibling
    )
  ...
  React.useEffect(() => {
    ...
    if (router.pathname === '/') {
      ...
      //
      const customWrappers = document.querySelectorAll(
        '.custom-wrapper-class, .custom-divider-wrapper'
      )
      ...
      notionLinks.forEach((notionLink) => {
        ...
        const parentWrapper = notionLink.closest(
          '.custom-wrapper-class, .custom-divider-wrapper'
        )

        if (parentWrapper) {
          // Clean up the page title text if it exists
          const pageTitleText = notionLink.querySelector(
  ...
  React.useEffect(() => {
    ...
    if (!parentContainer) {
      ...
      customWrappers[0].parentNode.insertBefore(
        parentContainer,
        customWrappers[0]
      )
    }
    ...
    customWrappers.forEach((wrapper) => {
      ...
      if (
        wrapper.parentNode !== parentContainer &&
        !wrapper.contains(parentContainer)
      ) {
