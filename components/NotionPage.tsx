...
React.useEffect(() => {
  ...
  const timeoutId = setTimeout(() => {
    ...
    notionLinks.forEach((link) => {
      ...
      type IconKey = 'LINK' | 'BOOK' | 'PLAY' | 'PROF';
      const ICON_METADATA: Record<IconKey, { path: string[]; bbox: { x: number; y: number; width: number; height: number } }> = {
        LINK: {
          path: LINK_ICON_PATH,
          bbox: {x: 1.9710676670074463, y: 2.029789924621582, width: 9.998832702636719, height: 9.998963356018066},
        },
        BOOK: {
          path: BOOK_ICON_PATH,
          bbox: {x: 2.5048201084136963, y: 1.6875, width: 8.99038028717041, height: 10.625}
        },
        PLAY: {
          path: PLAY_ICON_PATH,
          bbox: {x: 0, y: 0, width: 8, height: 10}
        },
        PROF: {
          path: PROF_ICON_PATH,
          bbox: {x: 1.303464651107788, y: 1.853760004043579, width: 8.727234840393066, height: 8.727210998535156}
        }
      }

      let iconKey: IconKey = "LINK";
      let viewBoxWidth = 16;
      ...
      const customIconPath = ICON_METADATA[iconKey].path;

      /* --- Center the SVG icon inside the circular background --- */

      // Since the icons are made up of multiple paths, we group them and apply transformations to the icon all at once
      ...
      // Calculate and apply the translation to center the bbox within the current viewBox
      const bbox = ICON_METADATA[iconKey].bbox;
      const translateX = (viewBoxWidth / 2) - (bbox.x + bbox.width / 2);
      const translateY = (viewBoxHeight / 2) - (bbox.y + bbox.height / 2);
